...
class PlayState extends MusicBeatState
{
	...
	['garbage (you suck too much dick to the point where i cant even grade it)', 0.2], //From 0% to 19%
	['stupid (F-)', 0.4], //From 20% to 39%
	['rip (F)', 0.5], //From 40% to 49%
	['Why (D)', 0.6], //From 50% to 59%
	['Uhhhh (C)', 0.69], //From 60% to 68%
	['Nice number (BEST NUMBER)', 0.7], //69%
	['Pretty ok (B)', 0.8], //From 70% to 79%
	['Good? (A)', 0.9], //From 80% to 89%
	['Pretty noice (S)', 1], //From 90% to 99%
	['pogosticks33 (SS)', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['part1_nomiss', 'debugged', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
	...
	private function checkForAchievement(achievesToCheck:Array<String> = null):String
	{
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'part1_nomiss' | 'week2_nomiss' | 'week3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'dotweek1':
									if(achievementName == 'part1_nomiss') unlock = true;
								case 'week2':
					...
					case 'debugger':
						...
						case 'debugged':
						if(Paths.formatToSongPath(SONG.song) == 'version-alpha' && !usedPractice) {
							unlock = true;
						}
				}
